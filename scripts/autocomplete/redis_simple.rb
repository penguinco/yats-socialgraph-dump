  ...
  File.new("screenname_followercount.csv").each_line{|n|
    ...
    (1..(n.length)).each{|l|
      ...
      if prefix.size <= FLIMIT
        ...
        #r.zadd(prefix,c.to_i,n[prefix.size, n.size]) #if you load at once, prefer zadd()
    ...
    r.zadd(:compl,0,n+"*")
    r.zincrby(:count,1,n) #if you load incrementaly, prefer zincrby()
    #r.zadd(:count,c.to_i,n) #if you load at once, prefer zadd()
  }
end
